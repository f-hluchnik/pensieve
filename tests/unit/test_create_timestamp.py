...
class TestCreateTimestamp(unittest.TestCase):
    ...
    def test_create_timestamp_valid(self):
        ...
        for test_date in test_dates:
            self.assertAlmostEqual(float(self.client.create_timestamp(test_date["date_string"])), float(test_date["timestamp"]), delta=1)
    
    def test_create_timestamp_invalid(self):
        test_dates = [{"date_string": "wrong"},
                      {"date_string": "123:20"}]
        for test_date in test_dates:
            self.assertIsNone(self.client.create_timestamp(test_date["date_string"]))
